...
if ( args$copy_number == 1){
 ...
 input_CpG_data_file <- "/Users/cdesouza/Documents/beast_share_lustre/Projects/BS-seq/whole_genome_single_cell/EPI-69/extracting_region_based_meth_data/CpGs_DMRs/DMRs_all_cells_input_Epiclomal_hg19_xeno7.tsv"
 input_regions_file <- "/Users/cdesouza/Documents/beast_share_lustre/Projects/BS-seq/whole_genome_single_cell/EPI-69/extracting_region_based_meth_data/CpGs_DMRs/DMRs_regionIDs_input_Epiclomal_hg19_xeno7.tsv"
 inferred_clusters_file <- "/Users/cdesouza/Documents/shahlab15/mandronescu/EPI-69_xeno7/OUTPUT_DMRs_K2-5_genesis/RUN/DMRs_K2-5_genesis_K2_epiclomal_real_basic_cn_region/outputs/epiclomal_basic/11/cluster_MAP.tsv.gz"
...
### very important that input_CpG_data has columns ordered by genomic coordinates, chr and position
mean_meth_matrix <- t(apply(input_CpG_data,1,extract_mean_meth_per_cell,region_coord=input_regions))

get_chr <- function(x,names){

  chr <- as.character(paste0("chr",unlist(strsplit(names[x[2]],":"))[1]))
  
}

region_chrs <- apply(input_regions,1,get_chr,names=colnames(input_CpG_data))
tmp <- t(region_chrs)
colnames(tmp) <- NULL
region_chrs <- as.vector(t(tmp))

##colnames(mean_meth_matrix) <- region_chrs
...
if (M > 250) {
  ...
  if (R > 1){
    ...
    ## annotating the columns by region
    reg_id <- region_chrs 
    annotation_col <- as.matrix(reg_id,nrow=length(colnames(mean_meth_matrix)))
    rownames(annotation_col) <- colnames(mean_meth_matrix)
    annotation_col <- as.data.frame(annotation_col)
    colnames(annotation_col) <- "chrs"
    annotation_col$chrs <- as.factor(annotation_col$chrs)
    
    annotation_col$chrs <- factor(annotation_col$chrs,level=c(paste0("chr",c(1:22,"X"))))
    ...
    pheatmap(tmp,cluster_cols=FALSE,
             ...
             #cellwidth = 5,cellheight = 5,
             fontsize = 8, main = paste0("Region-based mean methylation fraction data for ", "DMRs"),
             gaps_row = index_gaps,fontsize_row=8,fontsize_col=6,
             annotation_names_row = FALSE,
             annotation_col=annotation_col,
             #annotation_colors = ann_colors,
             border_color=NA,show_colnames=FALSE,
             filename = paste0(outdir,"DMRs_region_based_PLOT.pdf"))  
